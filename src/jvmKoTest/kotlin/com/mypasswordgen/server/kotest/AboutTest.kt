...
class AboutTest : FunSpec({
    val testTransaction = initDatabase()

    context("get about page") {
    ...
    context("get contribute page") {
        context("without cookie") {
            should("respond with plain text" and "status OK") {
                testApplication {
                    checkAll(FullDatabaseDto.arb(0..maxSize)) { fullDatabaseDto ->
                        testTransaction {
                            exec(fullDatabaseDto.insertStatement())
                        ...
                        response shouldHaveStatus HttpStatusCode.OK
                        response shouldHaveContentType ContentType.Text.Plain
                        testTransaction {
                            FullDatabaseDto.recoverFromDatabase() shouldBe databaseBefore
                            cleanUp()
                        }
